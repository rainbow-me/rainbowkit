...
Configurez vos chaînes souhaitées et générer les connecteurs requis. Vous aurez également besoin d'établir une configuration `wagmi`. If your dApp uses server side rendering (SSR) make sure to set `ssr` to `true`.
