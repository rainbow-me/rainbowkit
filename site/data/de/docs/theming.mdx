...
Eine Themenfunktion gibt ein Themenobjekt zurück. Sie können das Objekt an die `RainbowKitProvider`-`theme`-Eigenschaft übergeben.
...
Jedes Thema bietet auch mehrere vordefinierte Akzentfarben (`blue`, `green`, `orange`, `pink`, `purple`, `red`), die in das Optionsobjekt aufgenommen werden können. Zum Beispiel, um das `pink` Akzentfarb-Preset zu verwenden:
...
Standardmäßig ist der `fontStack` auf `rounded` eingestellt. Hier erfahren Sie, wie Sie die `fontStack`-Konfiguration verwenden können.
...
Standardmäßig ist die `overlayBlur` auf `none` eingestellt. Hier erfahren Sie, wie Sie die `overlayBlur`-Konfiguration verwenden können.
