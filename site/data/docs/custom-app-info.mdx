...
You can provide your own disclaimer. The (`disclaimer`) property in `appInfo` takes a `DisclaimerComponent` with args `Text` and `Link` that you can use to build your own disclaimer that inherits the theming system. We export the `DisclaimerComponent` type for TypeScript users:
