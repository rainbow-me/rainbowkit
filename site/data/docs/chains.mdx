...
Your chain config is defined in your [RPC Provider](/docs/rpc-providers) which gives you back a set of `chains` to pass through to `RainbowKitProvider`.

In the example below, we are using the Alchemy RPC Provider, but you can choose [another one](/docs/rpc-providers) if you prefer.

```tsx
import { RainbowKitProvider } from '@rainbow-me/rainbowkit';
import { chain } from 'wagmi';

const { chains } = rpcProvider.alchemy(
