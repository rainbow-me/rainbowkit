...
Added new required `initialChain` prop to `RainbowKitProvider`

RainbowKit previously (as of v0.3.2) automatically connected to the first chain in the `chains` prop on `RainbowKitProvider`. This behavior is no longer provided by default. Instead, you must explicitly specify which chain you want users to initially connect to.
