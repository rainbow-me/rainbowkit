...
<PropsTable
  data={[
    ...
    },
    {
      ...
      type: '() => void',
      description:
        'Set a callback that executes when wallet disconnect',
    },
  ]}
/>
...
Use the `onConnect` and `onDisconnect` props to set callbacks executed when wallet connects or disconnects.

When wallet connects.

```tsx
<ConnectButton onConnect={() => console.log('gm!')} />
```

When wallet disconnects.

```tsx
<ConnectButton onDisconnect={() => console.log('gn!')} />
```
