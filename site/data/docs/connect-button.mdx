...
> Note: Make sure rour app must is wrapped in the necessary providers. [Read more](/docs/installation#wrap-providers).
...
Here are a few different ways you can use the `balanceStatus` prop.

Hide the balance.

```tsx
<ConnectButton balanceStatus={false} />
```
