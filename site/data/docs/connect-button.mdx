...
<PropsTable
  data={[
    ...
    {
      name: 'onConnectChange',
      required: false,
      type: '({ isConnected: boolean, data?: ConnectorData }) => void',
      typeSimple: '(connection: ConnectionInfo) => void',
      description: 'Set a callback that executes when wallet connects',
    },
  ]}
/>
...
Use the `onConnectChange` props to set callbacks executed when wallet connects or disconnects. This callback will only get invoked after interacting with `ConnectButton`.
