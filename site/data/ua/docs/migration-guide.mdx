...
Примітка: wagmi більше не використовує пакет `ethers` внутрішньо. Але якщо ви покладаєтесь на API [Authentication](https://www.rainbowkit.com/docs/authentication), `siwe` все ще вимагатиме `ethers` як залежність.
