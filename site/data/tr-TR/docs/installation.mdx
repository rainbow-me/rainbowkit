...
İstediğiniz zincirleri yapılandırın ve gerekli bağlayıcıları oluşturun. Ayrıca bir `wagmi` konfigürasyonu kurmanız gerekecek. If your dApp uses server side rendering (SSR) make sure to set `ssr` to `true`.
