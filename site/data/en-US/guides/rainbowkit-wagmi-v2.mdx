...
// New RainbowKit configuration setup
...
Example usage with `getDefaultWallets` and `getDefaultConfig`.
...
Example usage with `getDefaultWallets`, `connectorsForWallets` and `createConfig`.
