...
You do need to pass parameters to `getDefaultWallets` to receive prepared `connectors` for Wagmi's `createConfig`. `chains` is no longer required.
```diff
const { connectors } = getDefaultWallets({
  appName: 'RainbowKit demo',
  projectId,
