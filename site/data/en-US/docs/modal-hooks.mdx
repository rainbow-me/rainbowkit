...
Each hook also returns a boolean for the status of the modal. It is typically recommended that you rely purely on Wagmi hooks (i.e. `useConnection`) to react to a user's wallet connection status directly, rather than relying on the state of the Connect Modal.
