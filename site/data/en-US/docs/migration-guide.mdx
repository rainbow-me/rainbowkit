...
Every dApp must now provide a [WalletConnect](https://cloud.reown.com/) `projectId` to enable WalletConnect v2. This must be completed before WalletConnect v1 bridge servers are shutdown on June 28, 2023. RainbowKit will quietly prefer v1 for all wallets if `projectId` is unspecified.
...
#### 2. Supply a WalletConnect projectId

Every dApp that relies on WalletConnect now needs to obtain a `projectId` from [Reown Cloud](https://cloud.reown.com/). This is absolutely free and only takes a few minutes.
