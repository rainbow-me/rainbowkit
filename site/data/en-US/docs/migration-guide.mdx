...
### @rainbow-me/rainbowkit@beta Breaking changes

#### Updated to wagmi v2

The [wagmi](https://wagmi.sh) peer dependency is migrated over to [wagmi v2](https://rc.wagmi.sh/) which has been updated to `^2.0.0`.
...
**2. Create custom wallet**

We've updated the way we create new wallets. Please refer to this [page](https://www.rainbowkit.com/docs/custom-wallets).

**3. WalletConnect legacy deprecation**

RainbowKit no longer supports WalletConnect legacy option. We now fully rely on the WalletConnect v2.

#### Rainbow Button migration

If you were using the rainbow button make sure to change `RainbowConnector` to `rainbowConnector` and remove `chains`. Apart from that nothing else is needed. You're good to go!
...
const connectors = connectorsForWallets([
  {
    ...
    wallets: [
-      new RainbowConnector({ chains, projectId }),
+      rainbowConnector({ projectId }),
    ],
  },
]);
```
