...
<PropsTable
  data={[
    ...
    {
      name: 'showCurrentChain',
      required: false,
      default: '{ smallScreen: false, largeScreen: true }',
      type: 'boolean | { smallScreen: boolean, largeScreen?: boolean }',
      typeSimple: 'enum',
      description:
        'Whether the current chain should be shown without having to open the chain modal',
    },
  ]}
/>
...
Use the `showCurrentChain` prop to always show the current chain without having to open the chain modal.

Show current chain.

```tsx
<ConnectButton showCurrentChain={__false__} />
```
...
On small screens, disable current chain. But on large screens, enable it.
