...
If you want to switch chains before connecting your wallet, just set `ignoreChainModalOnConnect` prop to `false` in `<RainbowKitProvider>`. This will let you interact with the chain modal before your wallet is connected, and also while it is connected.

```tsx
<RainbowKitProvider ignoreChainModalOnConnect={false}>
```
