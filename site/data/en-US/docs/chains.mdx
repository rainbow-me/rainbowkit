...
RainbowKit is designed to integrate with [wagmi’s `chain` object](https://wagmi.sh/react/api/chains). Check out the list of supported chains [here](https://wagmi.sh/react/api/chains#available-chains).

> For more detail about the `chain` object, or to see examples when creating a custom chain definition, see the [source code for wagmi and viem's `chain` object](https://github.com/wevm/viem/blob/main/src/types/chain.ts).
