...
By default your dApp uses public RPC providers to fetch things like balances, ENS name, and more. Which means you might not want to rely on the default public RPC providers because they can be unreliable sometimes. Instead you can define your own Transports. This can be done by adding `transports` in `getDefaultConfig`.

A Transport is the middle layer that handles sending JSON-RPC requests to the RPC Provider (like Alchemy, Infura, etc).
...
For more details, view the [wagmi transport documentation.](https://wagmi.sh/core/api/transports#transports)
