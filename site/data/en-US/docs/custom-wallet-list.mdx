...
To check whether the official WalletConnect modal is open, use the `useIsWalletConnectModalOpen` hook. It returns a boolean value true if open, false if closed. Import it directly from the `@rainbow-me/rainbowkit`.
