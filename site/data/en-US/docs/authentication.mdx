...
While it's possible to [integrate with custom back-ends and message formats,](/docs/custom-authentication) RainbowKit provides first-class support for [Sign-In with Ethereum](https://login.xyz) and [NextAuth (Auth.js)](https://authjs.dev).

### Set up Sign-In with Ethereum and NextAuth (Auth.js)
...
You can also pass down the resolved session object from the server via `getServerSideProps` so that NextAuth (Auth.js) doesn't need to resolve it again on the client.
