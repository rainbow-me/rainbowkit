...
`createMessage` também suporta retornar uma `Promise`, permitindo que você construa mensagens [EIP-4361](https://eips.ethereum.org/EIPS/eip-4361) no servidor. Para segurança mais rigorosa, o endpoint do servidor deve derivar ou validar campos críticos de segurança como `domain`, `nonce` e os timestamps de `issued-at` ao invés de confiar em valores fornecidos pelo cliente. Consulte a [documentação SIWE](https://docs.siwe.xyz/security-considerations#server-side-message-generation) para mais detalhes.
