...
Configure suas cadeias desejadas e gere os conectores necessários. Você também precisará configurar um `wagmi` config. If your dApp uses server side rendering (SSR) make sure to set `ssr` to `true`.
