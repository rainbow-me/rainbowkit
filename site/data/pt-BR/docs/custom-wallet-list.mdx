...
Para dApps que suportam Safe Apps para permitir que os usuários se conectem facilmente com seu cofre Safe ou multi-sig, inclua a opção `safeWallet` para conexões automáticas. A opção Safe só aparecerá no ambiente do navegador Safe Wallet.
...
Este conector de carteira suporta tanto o aplicativo e extensão da Coinbase Wallet, quanto a Coinbase Smart Wallet na Web.

Um argumento de `preferência` está disponível para controlar se a Smart Wallet está ativada e disponível para os usuários. O comportamento baseado em preferência está documentado [aqui](https://www.smartwallet.dev/sdk/makeWeb3Provider#parameters).

A Smart Wallet será ativada por padrão com `all` no início de junho, sem necessidade de atualização adicional.

Os desenvolvedores podem testar a Smart Wallet com as cadeias `sepolia` e `baseSepolia` hoje, configurando `smartWalletOnly` e incluindo `coinbaseWallet` em sua lista de carteiras assim:
