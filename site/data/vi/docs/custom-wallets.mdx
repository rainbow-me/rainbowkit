...
> Lưu ý: API này không ổn định và có thể thay đổi trong thời gian tới. Chúng tôi sẽ thêm nhiều ví được tích hợp sẵn qua thời gian. [Cho chúng tôi biết](https://github.com/rainbow-me/rainbowkit/issues) nếu có bất kỳ ví nào mà bạn quan tâm.

Loại hàm `Wallet` được cung cấp để giúp bạn định nghĩa ví tùy chỉnh của riêng mình. Các thuộc tính sau có thể được cấu hình trên giá trị trả về của hàm `Wallet` của bạn:
