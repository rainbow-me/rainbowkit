...
Lưu ý: wagmi không còn sử dụng gói `ethers` bên trong. Nhưng nếu bạn dựa vào API [Authentication](https://www.rainbowkit.com/docs/authentication), `siwe` sẽ vẫn yêu cầu `ethers` như một phụ thuộc đồng hành.
