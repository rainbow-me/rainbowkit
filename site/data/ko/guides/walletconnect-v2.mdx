...
WalletConnect를 의존하는 모든 dApp은 이제 [WalletConnect Cloud](https://cloud.walletconnect.com/)에서 `projectId`를 얻어야합니다. 이는 완전히 무료이며 몇분밖에 걸리지 않습니다.
...
당신의 dApp이 아직 WalletConnect v2로 마이그레이션하지 않은 레거시 월렛을 지원해야한다면, RainbowKit의 기본 WalletConnect 버전은 각 월렛별로 독립적으로 오버라이드 될 수 있습니다.
...
WalletConnect v1 서버가 6월 28일에 종료되면, v1 프로토콜을 계속 사용하기 위해서는 [사용자 정의 브릿지 서버](https://docs.walletconnect.com/1.0/bridge-server)가 필요합니다. 다음 예를 참조하십시오:
