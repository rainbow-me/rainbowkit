...
以下のウォレットは、特定のアプリではなく、一般的な接続方法に限定されています。 その結果、これらのウォレットは常に含めることをお勧めします。
...
**すべての注入されたウォレットがサポートされるように、常にこのウォレットをリストに含めることをお勧めします。**
...
このウォレットコネクタは、Coinbase Walletアプリと拡張機能、さらにWeb上のCoinbase Smart Walletの両方をサポートしています。

`preference`引数は、Smart Walletをユーザーにとって優先するか無効にするかを制御するために利用可能です。 プリファレンスに基づく動作は、[こちら](https://docs.base.org/identity/smart-wallet/technical-reference/sdk/#options-optional)で文書化されています。

[Paymasters](https://docs.base.org/identity/smart-wallet/guides/paymasters)および[サブアカウント](https://docs.base.org/identity/smart-wallet/guides/sub-accounts)の制御に関する追加の設定オプションは、Coinbase Wallet SDKのドキュメントに[こちら](https://docs.base.org/identity/smart-wallet/technical-reference/sdk#parameters)あります。
