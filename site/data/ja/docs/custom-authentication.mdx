...
`createMessage` は `Promise` を返すこともサポートしており、サーバー上で [EIP-4361](https://eips.ethereum.org/EIPS/eip-4361) メッセージを構築できます。より厳しいセキュリティを実現するために、サーバーエンドポイントは、クライアントが提供する値を信用せずに、`domain`、`nonce`、および `issued-at` タイムスタンプなどのセキュリティクリティカルなフィールドを導出または検証する必要があります。詳細については、[SIWE ドキュメント](https://docs.siwe.xyz/quickstart/creating-messages#server-side-message-creation) を参照してください。
