...
> 注: WalletConnectを利用するすべてのdAppは、現在[WalletConnect Cloud](https://cloud.walletconnect.com/)から`projectId`を取得する必要があります。 これは完全に無料で、数分で完了します。 これは完全に無料で、数分で完了します。
...
Next.jsとCreate React Appが使用する[Webpack v5](https://webpack.js.org/blog/2020-10-10-webpack-5-release/)バンドラは、[もはやNodeポリフィルを提供しない](https://webpack.js.org/blog/2020-10-10-webpack-5-release/#automatic-nodejs-polyfills-removed)ため、RainbowKitのピア依存関係を満たすためにはこれらのモジュールを自己負担する必要があります。 Create React Appでは特に`Buffer`をポリフィルする必要があります。 Create React Appでは特に`Buffer`をポリフィルする必要があります。

[ethers](https://docs.ethers.org/v5/)に依存していたRainbowKitの以前のバージョンでは、`fs`、`net`、そして`tls`モジュールが自動的にポリフィルされました。 これは、RainbowKit v1 + wagmi v1には当てはまらず、これらは [viem](https://viem.sh/)を基礎にして構築されています。 これは、RainbowKit v1 + wagmi v1には当てはまらず、これらは [viem](https://viem.sh/)を基礎にして構築されています。
...
Remixでは、`global`、`Buffer`、`process.env`をポリフィルする必要もあります。 Remixでは、`global`、`Buffer`、`process.env`をポリフィルする必要もあります。 [サンプル Remix プロジェクトの polyfills](https://github.com/rainbow-me/rainbowkit/blob/main/examples/with-remix/app/polyfills.ts)を参照してください。
