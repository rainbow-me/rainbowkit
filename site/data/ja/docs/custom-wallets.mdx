...
<PropsTable
  data={[
    ...
    },
    {
      ...
      description:
        'Function for calculating whether this wallet should be hidden from the list. これは、汎用のフォールバックウォレットを定義するために便利です。例えば、組み込みの「Injected Wallet」フォールバックは、この機能を使用しています。',
    },
    {
      ...
      description:
        'さまざまな接続方法のコネクタインスタンスと構成を提供するための関数。以下で説明します。',
    },
  ]}
/>
...
### RainbowKitConnectorのプロパティ

次のプロパティは`createConnector`関数の戻り値で定義されています。
