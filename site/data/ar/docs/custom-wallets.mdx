---
title: محافظ مخصصة
description: إنشاء محفظة مخصصة
---

# محافظ مخصصة

## إنشاء محفظة مخصصة

> ملاحظة: هذا الواجهة البرمجية للتطبيقات غير مستقرة ومرجح أن تتغير في المستقبل القريب. سنقوم بإضافة المزيد من المحافظ المدمجة مع مرور الوقت. [أعلمنا](https://github.com/rainbow-me/rainbowkit/issues) إذا كانت هناك أي محافظ معينة تهمك.

يتم توفير نوع الوظيفة `Wallet` لمساعدتك في تعريف محافظك المخصصة. يمكن تكوين الخصائص التالية على قيمة الإرجاع لوظيفة `Wallet` الخاصة بك:

### خصائص المحفظة

<PropsTable
  data={[
    {
      name: 'id',
      required: true,
      type: 'string',
      description: 'Unique ID per wallet',
    },
    {
      name: 'name',
      required: true,
      type: 'string',
      description: 'Human-readable wallet name',
    },
    {
      name: 'shortName',
      required: false,
      type: 'string | undefined',
      description: 'Optional short name for mobile use',
    },
    {
      name: 'iconUrl',
      required: true,
      type: 'string | (() => Promise<string>)',
      description:
        'رابط URL لرمز المحفظة، أو وعد يتحقق مع URL لبيانات Base64',
    },
    {
      name: 'iconAccent',
      required: false,
      type: 'string',
      description:
        'لون التمييز المستخدم في عمليات التنزيل للمحافظ التي لديها امتداد متصفح وتطبيق موبايل',
    },
    {
      name: 'iconBackground',
      required: true,
      type: 'string',
      description: 'لون الخلفية أثناء تحميل رمز المحفظة',
    },
    {
      name: 'installed',
      required: false,
      type: 'boolean | undefined',
      description: (
        <>
          إذا كانت المحفظة معروفة بأنها مثبتة، أو{' '}
          <code>غير محدد</code> إذا كان غير محدد
        </>
      ),
    },
    {
      name: 'downloadUrls',
      required: false,
      type: '{ android?: string, ios?: string, mobile?: string, qrCode?: string, chrome?: string, firefox?: string, edge?: string, safari?: string, opera?: string, browserExtension?: string } | undefined',
      typeSimple: 'DownloadUrls | undefined',
      description: 'كائن يحتوي على روابط التنزيل',
    },
    {
      name: 'hidden',
      required: false,
      type: '(args: { wallets: Array<{ id: string, connector: Connector, installed?: boolean }> }) => boolean',
      typeSimple: '(args) => boolean | undefined',
      description:
        'دالة لحساب ما إذا كان يجب إخفاء هذه المحفظة من القائمة. هذا مفيد لتعريف المحافظ العامة الاحتياطية، مثل الاستخدام الافتراضي لـ "المحفظة المدمجة" التي تستخدم هذه الميزة',
    },
    {
      name: 'createConnector',
      required: true,
      type: '() => RainbowKitConnector',
      typeSimple: 'RainbowKitConnector',
      description:
        'الدالة لتقديم نسخة الرابط و التكوين لطرق الاتصال المختلفة، وصف أدناه',
    },
  ]}
/>

### خصائص روابط التحميل

<PropsTable
  data={[
    {
      name: 'android',
      required: false,
      type: 'string',
      description: 'Google Play URL',
    },
    {
      name: 'ios',
      required: false,
      type: 'string',
      description: 'Apple App Store URL',
    },
    {
      name: 'mobile',
      required: false,
      type: 'string',
      description:
        'Landing page for mobile users when `android` or `ios` are unavailable',
    },
    {
      name: 'qrCode',
      required: false,
      type: 'string',
      description:
        'Landing page for users that scan the mobile download QR Code',
    },
    {
      name: 'chrome',
      required: false,
      type: 'string',
      description: 'Chrome Web Store URL',
    },
    {
      name: 'edge',
      required: false,
      type: 'string',
      description: 'Microsoft Edge Add-ons URL',
    },
    {
      name: 'firefox',
      required: false,
      type: 'string',
      description: 'Firefox Browser Add-ons URL',
    },
    {
      name: 'opera',
      required: false,
      type: 'string',
      description: 'Opera add-ons URL',
    },
    {
      name: 'safari',
      required: false,
      type: 'string',
      description: 'Mac App Store URL',
    },
    {
      name: 'browserExtension',
      required: false,
      type: 'string',
      description:
        'Landing page for desktop extension users when browser-compatible URLs are unavailable',
    },
  ]}
/>

### خصائص RainbowKitConnector

تم تعريف الخصائص التالية على قيمة العودة للدالة `createConnector`.

<PropsTable
  data={[
    {
      name: 'mobile',
      required: false,
      type: '{ getUri?: () => Promise<string> }',
      typeSimple: 'object',
      description:
        'الدالة لحل رابط اتصال المحفظة الجوال',
    },
    {
      name: 'desktop',
      required: false,
      type: '{ getUri?: () => Promise<string> }',
      typeSimple: 'object',
      description: 'الدالة لحل رابط التطبيق المكتبي العميق',
    },
    {
      name: 'qrCode',
      required: false,
      type: "{ getUri: () => Promise<string>, instructions?: { learnMoreUrl: string, steps: Array<{ step: 'install' | 'create' | 'scan', title: string, description: string }}}}",
      typeSimple: 'object',
      description:
        'كائن يحتوي على دالة لحل الرابط الرمزي الرباعي الأبعاد (QR) ، بالإضافة إلى تعليمات اختيارية لإعداد المحفظة الجوال',
    },
    {
      name: 'extension',
      required: false,
      type: "{ instructions?: { learnMoreUrl: string, steps: Array<{ step: 'install' | 'create' | 'scan', title: string, description: string }}}}",
      typeSimple: 'object',
      description:
        'كائن يحتوي على تعليمات اختيارية لإعداد الإضافة المتصفح',
    },
  ]}
/>

### أمثلة

مثلا، لإنشاء محفظة مخصصة باستخدام WalletConnect:

```tsx
import {
  Chain,
  Wallet,
  getWalletConnectConnector,
} from '@rainbow-me/rainbowkit';

export interface MyWalletOptions {
  projectId: string;
  chains: Chain[];
}

export const rainbow = ({
  chains,
  projectId,
}: MyWalletOptions): Wallet => ({
  id: 'my-wallet',
  name: 'My Wallet',
  iconUrl: 'https://my-image.xyz',
  iconBackground: '#0c2f78',
  downloadUrls: {
    android: 'https://play.google.com/store/apps/details?id=my.wallet',
    ios: 'https://apps.apple.com/us/app/my-wallet',
    chrome: 'https://chrome.google.com/webstore/detail/my-wallet',
    qrCode: 'https://my-wallet/qr',
  },
  createConnector: () => {
    const connector = getWalletConnectConnector({ projectId, chains });

    return {
      connector,
      mobile: {
        getUri: async () => {
          const provider = await connector.getProvider();
          const uri = await new Promise<string>(resolve =>
            provider.once('display_uri', resolve)
          );
          return uri;
        },
      },
      qrCode: {
        getUri: async () => {
          const provider = await connector.getProvider();
          const uri = await new Promise<string>(resolve =>
            provider.once('display_uri', resolve)
          );
          return uri;
        },
        instructions: {
          learnMoreUrl: 'https://my-wallet/learn-more',
          steps: [
            {
              description:
                'We recommend putting My Wallet on your home screen for faster access to your wallet.',
              step: 'install',
              title: 'Open the My Wallet app',
            },
            {
              description:
                'After you scan, a connection prompt will appear for you to connect your wallet.',
              step: 'scan',
              title: 'Tap the scan button',
            },
          ],
        },
      },
      extension: {
        instructions: {
          learnMoreUrl: 'https://my-wallet/learn-more',
          steps: [
            {
              description:
                'We recommend pinning My Wallet to your taskbar for quicker access to your wallet.',
              step: 'install',
              title: 'Install the My Wallet extension',
            },
            {
              description:
                'Be sure to back up your wallet using a secure method. Never share your secret phrase with anyone.',
              step: 'create',
              title: 'Create or Import a Wallet',
            },
            {
              description:
                'Once you set up your wallet, click below to refresh the browser and load up the extension.',
              step: 'refresh',
              title: 'Refresh your browser',
            },
          ],
        },
      },
    };
  },
});
```

يمكنك بعد ذلك إضافة محفظتك المخصصة في [قائمة المحفظة](/docs/custom-wallet-list).
