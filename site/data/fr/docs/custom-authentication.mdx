...
`createMessage` prend également en charge le retour d'une `Promise`, vous permettant de construire des messages [EIP-4361](https://eips.ethereum.org/EIPS/eip-4361) sur le serveur. Pour une sécurité plus stricte, le point de terminaison du serveur devrait dériver ou valider des champs critiques pour la sécurité comme le `domaine`, le `nonce` et les horodatages `émis-à` plutôt que de se fier aux valeurs fournies par le client. Consultez la [documentation SIWE](https://docs.siwe.xyz/security-considerations#server-side-message-generation) pour plus de détails.
