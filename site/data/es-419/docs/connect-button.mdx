---
title: ConnectButton
description: Usando y personalizando el ConnectButton
---

# ConnectButton

## Usando y personalizando el ConnectButton

Este es el componente principal. Es responsable de renderizar el botón de conectar/desconectar, así como la interfaz de usuario para cambiar de cadena.

```tsx
import { ConnectButton } from '@rainbow-me/rainbowkit';

export const YourApp = () => {
  return <ConnectButton />;
};
```

> Nota: Asegúrese de que su aplicación está envuelta en los proveedores necesarios. [Lea más](/docs/installation#wrap-providers).

### Props

El componente `ConnectButton` expone varias props para personalizar su apariencia, incluyendo el cambio de visibilidad de diferentes elementos.

<PropsTable
  data={[
    {
      name: 'label',
      required: false,
      default: 'Conectar Billetera',
      type: 'string',
      description: 'La etiqueta de texto mostrada en el botón Conectar',
    },
    {
      name: 'accountStatus',
      required: false,
      default: 'completo',
      type: '"avatar" | "address" | "full" | { smallScreen: AccountStatus, largeScreen?: AccountStatus }',
      typeSimple: 'enum',
      description:
        'Si se muestra el avatar y/o la dirección de la cuenta activa',
    },
    {
      name: 'chainStatus',
      required: false,
      default: '{ smallScreen: "icon", largeScreen: "full" }',
      type: '"icon" | "name" | "full" | "none" | { smallScreen: ChainStatus, largeScreen?: ChainStatus }',
      typeSimple: 'enum',
      description:
        'Si se muestra o se oculta completamente el icono y/o el nombre de la cadena actual',
    },
    {
      name: 'showBalance',
      required: false,
      default: '{ smallScreen: false, largeScreen: true }',
      type: 'boolean | { smallScreen: boolean, largeScreen?: boolean }',
      typeSimple: 'enum',
      description:
        'Si el saldo es visible junto al nombre de la cuenta',
    },
  ]}
/>

> Nota: el punto de interrupción `largeScreen` incorporado es `768px`.

### Ejemplos

#### Etiqueta

Utilice la prop `label` para establecer un texto personalizado para `ConnectButton`.

```tsx
<ConnectButton label="__Sign in__" />
```

#### Estado de la cuenta

Aquí hay algunas maneras diferentes en las que puede usar la prop `accountStatus`.

Mostrar solo el avatar de la cuenta.

```tsx
<ConnectButton accountStatus="__avatar__" />
```

Mostrar solo la dirección de la cuenta.

```tsx
<ConnectButton accountStatus="__address__" />
```

#### Estado de la cadena

Aquí hay algunas maneras diferentes en las que puede usar la prop `chainStatus`.

Mostrar solo el ícono de la cadena actual.

```tsx
<ConnectButton chainStatus="__icon__" />
```

Mostrar solo el nombre de la cadena actual.

```tsx
<ConnectButton chainStatus="__name__" />
```

Ocultar completamente la interfaz de la cadena.

```tsx
<ConnectButton chainStatus="__none__" />
```

#### Mostrar saldo

Use la prop `showBalance` para ocultar/mostrar el saldo.

Ocultar el saldo.

```tsx
<ConnectButton showBalance={__false__} />
```

#### Responsivo

Estas props también pueden ser definidas en un formato responsivo.

En pantallas pequeñas, mostrar solo el icono de la cuenta. Pero en pantallas grandes, mostrar el icono y la dirección.

```tsx
<ConnectButton
  accountStatus={{
    smallScreen: 'avatar',
    largeScreen: 'full',
  }}
/>
```

En pantallas pequeñas, ocultar el saldo. Pero en pantallas grandes, mostrarlo.

```tsx
<ConnectButton
  showBalance={{
    smallScreen: false,
    largeScreen: true,
  }}
/>
```
