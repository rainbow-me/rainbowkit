...
Para dApps que soportan Safe Apps y permiten a los usuarios conectarse fácilmente con su bóveda Safe o multi-firma, incluye la opción `safeWallet` para conexiones automáticas. La opción de Safe solo aparecerá en el entorno del navegador Safe Wallet.
...
Este conector de billetera es compatible tanto con la aplicación y la extensión de Coinbase Wallet, como con el Smart Wallet de Coinbase en la Web.

Está disponible un argumento de `preference` para controlar si el Smart Wallet está habilitado y disponible para los usuarios. El comportamiento basado en preferencias está documentado [aquí](https://www.smartwallet.dev/sdk/makeWeb3Provider#parameters).

El Smart Wallet estará habilitado por defecto con `all` a principios de junio, sin una actualización adicional.

Los desarrolladores pueden probar el Smart Wallet con las cadenas `sepolia` y `baseSepolia` hoy configurando `smartWalletOnly` e incluyendo `coinbaseWallet` en su lista de carteras así:
