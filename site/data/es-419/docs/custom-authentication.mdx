...
`createMessage` también admite devolver una `Promise`, lo que te permite construir mensajes [EIP-4361](https://eips.ethereum.org/EIPS/eip-4361) en el servidor. Para una seguridad más estricta, el punto final del servidor debe derivar o validar campos críticos de seguridad como `domain`, `nonce` y las marcas de tiempo `issued-at`, en lugar de confiar en valores proporcionados por el cliente. Consulta la [documentación de SIWE](https://docs.siwe.xyz/security-considerations#server-side-message-generation) para más detalles.
