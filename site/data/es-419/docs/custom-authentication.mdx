...
#### Creación de mensajes del lado del servidor

`createMessage` también admite el retorno de una `Promise`, lo que te permite construir mensajes [EIP-4361](https://eips.ethereum.org/EIPS/eip-4361) en el servidor. Para una mayor seguridad, el endpoint del servidor debe derivar o validar campos críticos de seguridad como `domain`, `nonce` y las marcas de tiempo de `issued-at`, en lugar de confiar en los valores proporcionados por el cliente. Consulta la [documentación de SIWE](https://docs.siwe.xyz/quickstart/creating-messages#server-side-message-creation) para obtener más detalles.
