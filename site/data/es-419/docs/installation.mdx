...
Configure sus cadenas deseadas y genere los conectores requeridos. También necesitará configurar una `wagmi` config. If your dApp uses server side rendering (SSR) make sure to set `ssr` to `true`.
