...
Support for WalletConnect v2 is now standard in RainbowKit, without breaking changes. Just upgrade and follow the guide below!
...
RainbowKit enabled WalletConnect v2 by default with `0.12.15` and `1.0.2`. We are continuing support for `0.12.x` in case your dApp has not yet upgraded to Wagmi and RainbowKit v1.
...
It is recommended that `0.12.x` dApps also begin to upgrade, as improvements to WalletConnect v2 will only be included in future versions of Wagmi.
...
Once the WalletConnect v1 servers are shutdown on June 28th, a [custom bridge server](https://docs.walletconnect.com/1.0/bridge-server) is required to continue using the v1 protocol. The dApp and wallet must each utilize the same bridge server. Reference the following examples:
