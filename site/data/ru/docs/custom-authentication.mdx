...
#### Создание серверного сообщения

Функция `createMessage` также поддерживает возврат `Promise`, что позволяет создавать сообщения [EIP-4361](https://eips.ethereum.org/EIPS/eip-4361) на сервере. Для обеспечения более строгой безопасности серверный конечный пункт должен определять или проверять критически важные для безопасности поля, такие как `domain`, `nonce` и метки времени `issued-at`, вместо того чтобы доверять значениям, предоставленным клиентом. Для получения дополнительной информации см. [Документацию SIWE](https://docs.siwe.xyz/quickstart/creating-messages#server-side-message-creation).
