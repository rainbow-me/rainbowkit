...
Теперь каждому dApp, который использует WalletConnect, нужно получить `projectId` от [WalletConnect Cloud](https://cloud.walletconnect.com/). Это абсолютно бесплатно и занимает всего несколько минут.
...
Если ваш dApp требует поддержки устаревшего кошелька, который еще не перешел на WalletConnect v2, версию WalletConnect по умолчанию в RainbowKit можно переопределить для каждого кошелька отдельно.
...
После того как серверы WalletConnect v1 будут отключены 28 июня, для продолжения использования протокола v1 требуется [кастомный сервер моста](https://docs.walletconnect.com/1.0/bridge-server). Обратитесь к следующим примерам:
