...
此包兼容 Next.js、React 和 Vite。 确保遵循对等依赖的警告。 确保遵循对等依赖的警告。 确保遵循对等依赖的警告。
...
将 `rainbowConnector` 的一个实例传递给 Wagmi 的 `createConfig`，并传入 `projectId` 和 `appName`，然后将您的应用包装在 `RainbowButtonProvider` 中。 然后加入 `RainbowButton` 组件。 然后加入 `RainbowButton` 组件。 然后加入 `RainbowButton` 组件。
