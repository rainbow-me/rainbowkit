...
配置您所需的链并生成所需的连接器。 配置您所需的链并生成所需的连接器。 您还需要设置一个 `wagmi` 配置。

> 注意：现在依赖 WalletConnect 的每一个 dApp 都需要从 [WalletConnect Cloud](https://cloud.walletconnect.com/) 获得一个 `projectId`。 这完全免费，只需要几分钟。 这完全免费，只需要几分钟。 这完全免费，只需要几分钟。
...
在依赖 [ethers](https://docs.ethers.org/v5/) 的 RainbowKit 的先前版本中，`fs`、`net` 和 `tls` 模块会自动进行 polyfill。 在基于 [viem](https://viem.sh/) 的 RainbowKit v1 + wagmi v1 中，情况不再如此。 在基于 [viem](https://viem.sh/) 的 RainbowKit v1 + wagmi v1 中，情况不再如此。 在基于 [viem](https://viem.sh/) 的 RainbowKit v1 + wagmi v1 中，情况不再如此。
