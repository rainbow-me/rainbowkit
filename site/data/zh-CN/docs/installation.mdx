...
在依赖 [ethers](https://docs.ethers.org/v5/) 的 RainbowKit 的先前版本中，`fs`、`net` 和 `tls` 模块会自动进行 polyfill。 在基于 [viem](https://viem.sh/) 的 RainbowKit v1 + wagmi v1 中，情况不再如此。 在基于 [viem](https://viem.sh/) 的 RainbowKit v1 + wagmi v1 中，情况不再如此。
...
Remix 还要求您为 `global`，`Buffer` 和 `process.env` 填充兼容性代码。 Remix 还要求您为 `global`，`Buffer` 和 `process.env` 填充兼容性代码。 参考我们的样本 Remix 项目中的 [polyfills](https://github.com/rainbow-me/rainbowkit/blob/main/examples/with-remix/app/polyfills.ts)。
