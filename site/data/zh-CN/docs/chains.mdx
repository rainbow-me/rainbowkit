...
RainbowKit 旨在与 [wagmi 的 `chain` 对象](https://wagmi.sh/react/chains) 集成。 查看支持的链列表 [here](https://wagmi.sh/react/chains#supported-chains)。 查看支持的链列表 [here](https://wagmi.sh/react/chains#supported-chains)。 查看支持的链列表 [here](https://wagmi.sh/react/chains#supported-chains)。
...
默认情况下，RainbowKit将连接到您的`chains`数组中的第一条链，以确保用户不会立即面临"错误网络"状态。 可以通过`initialChain`属性自定义此行为。 可以通过`initialChain`属性自定义此行为。 可以通过`initialChain`属性自定义此行为。
