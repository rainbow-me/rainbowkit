...
如果有任何挂起的交易，`ConnectButton`的默认实现还将在用户的头像周围显示一个加载指示器。 自定义 `ConnectButton` 实现可以通过传递给您的渲染函数的 `account.hasPendingTransactions` 属性重现此行为。 自定义 `ConnectButton` 实现可以通过传递给您的渲染函数的 `account.hasPendingTransactions` 属性重现此行为。
