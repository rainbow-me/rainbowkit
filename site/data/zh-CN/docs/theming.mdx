...
您可以调整 RainbowKit UI 以匹配您的品牌。 您可以从几个预定义的重点色彩和边界半径配置中选择。 对于更高级的用例，您可以提供一个完全自定义的主题，渲染您自己的按钮并省略某些功能。 包括暗模式。 您可以调整 RainbowKit UI 以匹配您的品牌。 您可以从几个预定义的重点色彩和边界半径配置中选择。 您可以从几个预定义的重点色彩和边界半径配置中选择。
...
主题函数返回一个主题对象。 主题函数返回一个主题对象。 您可以将该对象传递给 `RainbowKitProvider` 的 `theme` 属性。 主题函数返回一个主题对象。 您可以将该对象传递给 `RainbowKitProvider` 的 `theme` 属性。
...
每个主题也提供了几种强调色彩预设（`blue`, `green`, `orange`, `pink`, `purple`, `red`）可以扩展到选项对象中。 例如，使用 `pink` 强调色预设： 例如，使用 `pink` 强调色预设： 例如，使用 `pink` 强调色预设：
...
默认情况下，`fontStack` 设置为 `rounded`。 但是这是你如何使用 `fontStack` 配置的方法。 但是这是你如何使用 `fontStack` 配置的方法。 但是这是你如何使用 `fontStack` 配置的方法。
...
默认情况下，`overlayBlur` 设置为 `none`。 但这是你可以如何使用 `overlayBlur` 配置的方法。 但这是你可以如何使用 `overlayBlur` 配置的方法。 但这是你可以如何使用 `overlayBlur` 配置的方法。
