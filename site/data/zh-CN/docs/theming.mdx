...
您可以调整 RainbowKit UI 以匹配您的品牌。 您可以从几个预定义的重点色彩和边界半径配置中选择。 您可以从几个预定义的重点色彩和边界半径配置中选择。
...
主题函数返回一个主题对象。 主题函数返回一个主题对象。 您可以将该对象传递给 `RainbowKitProvider` 的 `theme` 属性。
...
每个主题也提供了几种强调色彩预设（`blue`, `green`, `orange`, `pink`, `purple`, `red`）可以扩展到选项对象中。 例如，使用 `pink` 强调色预设： 例如，使用 `pink` 强调色预设：
