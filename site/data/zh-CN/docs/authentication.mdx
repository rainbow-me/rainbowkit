...
您可以使用NextAuth的 `getToken` 函数从 `next-auth/jwt` 导入会话令牌。 如果用户成功进行身份验证，会话令牌的 <b>`sub`</b> 属性（令牌的<b>"主题"</b>，即用户）将是用户的地址。 如果用户成功进行身份验证，会话令牌的 <b>`sub`</b> 属性（令牌的<b>"主题"</b>，即用户）将是用户的地址。 如果用户成功进行身份验证，会话令牌的 <b>`sub`</b> 属性（令牌的<b>"主题"</b>，即用户）将是用户的地址。
