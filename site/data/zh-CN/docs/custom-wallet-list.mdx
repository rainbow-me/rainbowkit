...
可以在[这里](https://docs.walletconnect.com/1.0/quick-start/dapps/web3-provider#optional)找到支持的 `options` 的额外文档 。
...
这是一个后备钱包选项，专为`window.ethereum`存在但列表中其他钱包未提供的情况设计。 如果不需要后备或者没有可用的注入钱包，这个钱包会自动从列表中隐藏自己。 如果不需要后备或者没有可用的注入钱包，这个钱包会自动从列表中隐藏自己。 如果不需要后备或者没有可用的注入钱包，这个钱包会自动从列表中隐藏自己。
