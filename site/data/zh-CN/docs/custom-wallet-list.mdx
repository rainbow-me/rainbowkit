...
这是一个后备钱包选项，专为`window.ethereum`存在但列表中其他钱包未提供的情况设计。 如果不需要后备或者没有可用的注入钱包，这个钱包会自动从列表中隐藏自己。
...
提供了一个`preference`参数，用于控制是否为用户启用和提供智能钱包。 基于偏好的行为记录于[此处](https://docs.base.org/identity/smart-wallet/technical-reference/sdk/#options-optional)。

在Coinbase Wallet SDK文档的[此处](https://docs.base.org/identity/smart-wallet/technical-reference/sdk#parameters)中可以找到控制[支付者](https://docs.base.org/identity/smart-wallet/guides/paymasters)和[子账户](https://docs.base.org/identity/smart-wallet/guides/sub-accounts)的其他配置选项。
