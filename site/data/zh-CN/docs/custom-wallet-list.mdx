...
A `preference` argument is available to control whether Smart Wallet is preferred or disabled for your users. 提供了一个`preference`参数，用于控制是否为用户启用和提供智能钱包。 基于偏好的行为记录于[此处](https://www.smartwallet.dev/sdk/makeWeb3Provider#parameters)。
