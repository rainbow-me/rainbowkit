...
#### Base（原 Coinbase Wallet）

这个钱包连接器支持Base，作为Coinbase智能钱包的升级版。它提供无缝身份认证和增强功能，如子账户和消费权限。

配置选项与[Base SDK 参考文档](https://docs.base.org/base-account/reference/core/createBaseAccount)保持一致。

> `baseAccount` 和 `coinbaseWallet` 仍可作为向后兼容的别名使用。
...
显示Rainbow、MetaMask和Base以及一般的备选钱包。
...
推荐Rainbow和MetaMask，但也提供Base以及一般的备选钱包。
