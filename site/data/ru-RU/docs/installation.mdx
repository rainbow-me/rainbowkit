...
Настройте нужные вам цепочки и сгенерируйте необходимые коннекторы. Вам также потребуется настроить конфиг `wagmi`. If your dApp uses server side rendering (SSR) make sure to set `ssr` to `true`.
