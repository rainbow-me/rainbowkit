...
開發者仍擁有完全控制自定義錢包清單的權限，以強調優先的錢包給終端用戶。 建議您繼續在列表中包含 `injectedWallet` 和 `walletConnectWallet` 以支持所有平台。 建議您繼續在列表中包含 `injectedWallet` 和 `walletConnectWallet` 以支持所有平台。 鼓勵您繼續在清單中包括 `injectedWallet` 和 `walletConnectWallet` 來支援所有平臺。
