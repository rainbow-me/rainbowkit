...
RainbowKit 設計是不打擾且具回應性的，所以無論使用者是否連接了他們的錢包，dApp 應該始終渲染一個界面給使用者。 使用者可以直接從 MetaMask 連接或斷開他們的錢包，所以 dApp 必須直接回應底層的連接狀態。 使用者可以直接從 MetaMask 連接或斷開他們的錢包，所以 dApp 必須直接回應底層的連接狀態。 使用者可以直接從 MetaMask 連接或斷開他們的錢包，所以 dApp 必須直接回應底層的連接狀態。
