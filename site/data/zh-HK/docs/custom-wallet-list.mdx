...
可用的 `preference` 參數可以控制是否優先或禁用 Smart Wallet 給用戶。 可用的 `preference` 參數可以控制是否優先或禁用 Smart Wallet 給用戶。 基於偏好的行為記錄在 [這裡](https://docs.base.org/identity/smart-wallet/technical-reference/sdk/#options-optional)。

有關 Paymasters 和 Sub Accounts 的額外配置選項可在這裡的 Coinbase Wallet SDK 文檔中找到 [這裡](https://docs.base.org/identity/smart-wallet/technical-reference/sdk#parameters)。
