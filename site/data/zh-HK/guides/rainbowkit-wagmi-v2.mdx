...
開發者仍擁有完全控制自定義錢包清單的權限，以強調優先的錢包給終端用戶。 建議您繼續在列表中包含 `injectedWallet` 和 `walletConnectWallet` 以支持所有平台。
...
RainbowKit 錢包連接器已經過重大變更以支援 Wagmi v2。 參考[更新的文件](https://www.rainbowkit.com/docs/custom-wallets)和一個[範例連接器](https://github.com/rainbow-me/rainbowkit/blob/main/packages/rainbowkit/src/wallets/walletConnectors/rainbowWallet/rainbowWallet.ts)來升級您在 dApp 中的任何自定義錢包連接器。
