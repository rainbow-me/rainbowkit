...
該套件兼容Next.js, React和Vite。 確保您遵循對等依賴警告。 確保您遵循對等依賴警告。 確保您遵循對等依賴警告。
...
將 `rainbowConnector` 的實例與`projectId`和`appName`一起傳遞給Wagmi的`createConfig`，並將您的應用包裹在`RainbowButtonProvider`中。 然後插入 `RainbowButton` 元件。 然後插入 `RainbowButton` 元件。 然後插入 `RainbowButton` 元件。
