...
將 `rainbowConnector` 的實例與 `projectId` 和 `appName` 一起傳遞給 Wagmi 的 `createConfig`，並將您的應用包裹在 `RainbowButtonProvider` 中。 然後放入 `RainbowButton` 組件。 然後放入 `RainbowButton` 組件。 然後放入 `RainbowButton` 組件。
