...
開發者繼續擁有完全控制自定義錢包列表的權限，以強調終端用戶的首選錢包。 開發者繼續擁有完全控制自定義錢包列表的權限，以強調終端用戶的首選錢包。 建議您繼續包括 `injectedWallet` 和 `walletConnectWallet` 在您的列表中，以支援所有平臺。 我們鼓勵您繼續在列表中包括`injectedWallet`和`walletConnectWallet`以支持所有平台。
