...
RainbowKit 被設計為非中斷和響應式，因此無論用戶是否已連接其錢包，dApp 都應始終為用戶渲染界面。 用戶可以直接從 MetaMask 連接或斷開他們的錢包，因此 dApp 必須直接響應底層連接狀態。 用戶可以直接從 MetaMask 連接或斷開他們的錢包，因此 dApp 必須直接響應底層連接狀態。 用戶可以直接從 MetaMask 連接或斷開他們的錢包，因此 dApp 必須直接響應底層連接狀態。
