...
可以使用 `preference` 參數控制是否為用戶啟用和提供 Smart Wallet。 可以使用 `preference` 參數控制是否為用戶啟用和提供 Smart Wallet。 `Preference` 參數可用於控制 Smart Wallet 是否為用戶啟用和可用。 基於偏好的行為在[此處](https://www.smartwallet.dev/sdk/makeWeb3Provider#parameters)有記錄。
