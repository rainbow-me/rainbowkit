...
- 7a24ced: Resolved an issue where the MetaMask button would not fallback to an available wallet provider if MetaMask was not active. Now you can continue to use the MetaMask button to interact with all wallets, or directly with MetaMask when it is active.
