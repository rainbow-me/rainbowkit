 ...
 */
export const useExplorerTxHistory = <
  Tx = any,
  P extends BaseProvider = Web3Provider
>({
  fetcher = logsFetcher,
  address,
  options,
  ...
  useEffect(() => {
    const sub: Listener = tx =>
      setData(arr => [...arr, { ...tx, status: 'pending' }]);

    if (address && provider && fetcher) {
      fetcher({ address, options, provider })
        .then(txes => {
          setLoading(false);
          setData(txes as Tx[]);
        })
